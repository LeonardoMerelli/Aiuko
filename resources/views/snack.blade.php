...
<div class="snack-div">
  ...
  <form action="{{route('snack.store')}}" method="post">
    ...
    <select name="diet[]" class="custom-select" id="snackSelect" multiple>
      @if($preferenzeSnack != null && $preferenzeSnack['faSpuntino'] == 1)
      <option value="si" selected>si</option>
      @else
      <option value="si">si</option>
      @endif
      @if($preferenzeSnack != null && $preferenzeSnack['faSpuntino'] == 0)   
      <option value="no" selected>no</option>
      @else
      <option value="no">no</option>
      @endif
      @if($preferenzeSnack != null && $preferenzeSnack['colazionePranzo'] == 1)
      <option value="colazione pranzo" selected>colazione pranzo</option>
      @else
      <option value="colazione pranzo">colazione pranzo</option>
      @endif
      @if($preferenzeSnack != null && $preferenzeSnack['pranzoCena'] == 1)
      <option value="pranzo cena" selected>pranzo cena</option>
      @else
      <option value="pranzo cena">pranzo cena</option>
      @endif
      @if($preferenzeSnack != null && $preferenzeSnack['dopoCena'] == 1)
